...
class GUI extends TopLevel implements Observer, Reporter {
    ...
    GUI(String title, Board model, Writer outCommands) {
        ...
        addMenuRadioButton("Options->Settings->Black AI", "Black",
                true, this::blackAI);
        addMenuRadioButton("Options->Settings->White AI", "White",
                false, this::whiteAI);
        addMenuRadioButton("Options->Settings->Black Manual", "Black",
                false, this::blackManual);
        addMenuRadioButton("Options->Settings->White Manual", "White",
                true, this::whiteManual);
        _model = model;
    ...
    /** Execute the "BlackManual" button function. */
    private synchronized void blackManual(String unused) {
        _out.printf("manual black%n");
    }
    /** Execute the "WhiteManual" button function. */
    ...
    public void update(Observable obs, Object arg) {
        if (obs == _model) {
            ...
            if (_selectedMove == null) {
                if (_model.get(c, r) != _model.whoseMove()) {
                    return;
                } else {
                    Move m = Move.move(c, r);
                    selectMove(m);
                }
                _widget.repaint();
            } else {
                Move m = Move.move(_selectedMove.col0(),
                        _selectedMove.row0(), c, r);
                if (_model.legalMove(m)) {
                    movePiece(sq);
                }
    ...
    private void movePiece(String sq) {
        char c = sq.charAt(0);
        char r = sq.charAt(1);
        Move m = Move.move(_selectedMove.col0(), _selectedMove.row0(), c, r);
        _out.printf("" + _selectedMove.col0() + _selectedMove.row0()
                + "-" + c + r + "%n");
        _widget.repaint();
        setChanged();
        notifyObservers();
    ...
    private void selectMove(Move mov) {
        if (_selectedMove == null) {
            ...
            _selectedMove = Move.move(_selectedMove, mov);
        }
